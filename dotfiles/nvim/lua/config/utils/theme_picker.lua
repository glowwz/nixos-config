...
function M.open()
	...
	Snacks.picker.pick({
		...
		format = function(item)
			local result = { { "  " .. item.text .. " ", "Normal" } }

			local ok, colors = pcall(function()
				return require("base46").get_theme_tb("base_30", item.text)
			end)
			...
			if ok and colors then
				...
				for _, color in ipairs(swatches) do
					if color then
						local hl_name = "ThemeSwatch_" .. color:gsub("#", "")
						vim.api.nvim_set_hl(0, hl_name, { fg = color })
						table.insert(result, { " ●", hl_name })
					end
				end
			end
